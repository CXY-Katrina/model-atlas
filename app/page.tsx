...
function denseGraph(layer: number): Record<string, OpNode> {
  ...
  return {
    ...
    postnorm: cloneOp(norm,{id:"d-postnorm",kind:"norm",title:"Post-attn Gemma RMSNorm",summary:"图中的 Add 节点先计算 U=Xₗ+Yattn；此节点只执行 Gemma RMSNorm(U)，输出唯一的 Û 作为 FFN 输入。",input:"U",inputShape:"[B,S,6144]",output:"Û",outputShape:"[B,S,6144]",source:"nvidia/model.py · MiniMAXGemmaRMSNorm.forward · L130–142",sourceUrl:NORM_FORWARD_URL,weights:[postNorm]}),
...
function sparseGraph(layer: number): Record<string, OpNode> {
  ...
  return {
    ...
    postnorm:cloneOp(normBase,{id:"s-postnorm",kind:"norm",title:"Post-attn Gemma RMSNorm",summary:"图中的 Add 节点先计算 U=Xₗ+Yattn；此节点只执行 Gemma RMSNorm(U)，输出唯一的 Û 作为 MoE 输入。",input:"U",inputShape:"[B,S,6144]",output:"Û",outputShape:"[B,S,6144]",source:"nvidia/model.py · MiniMAXGemmaRMSNorm.forward · L130–142",sourceUrl:NORM_FORWARD_URL,weights:[postNorm]}),

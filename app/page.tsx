...
function InputWeightedOp({node,active,onHover,onLeave,onSelect,inputName,inputShape,label="γ",inputGraphId,graphId,weightGraphId,className=""}:{node:OpNode;active:boolean;onHover:(n:OpNode)=>void;onLeave:()=>void;onSelect:(n:OpNode)=>void;inputName:string;inputShape:string;label?:string;inputGraphId:string;graphId:string;weightGraphId:string;className?:string}){
  const weight=node.weights[0];
  const symbolicWeightShape=weight?.shape.replaceAll("6144","H")??"[H]";
  return <div className={`input-weighted-op ${className}`}><div className="co-input-row"><Tensor name={inputName} shape={inputShape} graphId={inputGraphId}/><Tensor name={label} shape={symbolicWeightShape} role="weight" graphId={weightGraphId}/></div><Op node={node} active={active} onHover={onHover} onLeave={onLeave} onSelect={onSelect} graphId={graphId}/></div>;
}
...
function StageZoom({type,stage,g,active,onHover,onLeave,onSelect,onClose}:{type:LayerType;stage:Exclude<ExpandedStage,null>;g:Record<string,OpNode>;active:string;onHover:(n:OpNode)=>void;onLeave:()=>void;onSelect:(n:OpNode)=>void;onClose:()=>void}){
  ...
  const IW=({id,inputName,inputShape,label,inputGraphId,graphId,weightGraphId,className}:{id:string;inputName:string;inputShape:string;label?:string;inputGraphId:string;graphId:string;weightGraphId:string;className?:string})=><InputWeightedOp node={g[id]} {...p} active={active===g[id].id} inputName={inputName} inputShape={inputShape} label={label} inputGraphId={inputGraphId} graphId={graphId} weightGraphId={weightGraphId} className={className}/>;

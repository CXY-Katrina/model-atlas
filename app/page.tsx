...
function sparseGraph(layer: number): Record<string, OpNode> {
  ...
  return {
    ...
    cache:cloneOp(attn,{id:"s-cache",kind:"cache",title:"Paged KV Cache",input:"Kᵣ,V + block table",inputShape:"KV pages + [B,Nblocks]",output:"Top-16 selected pages",outputShape:"≤2048 KV tokens / group",formula:"physical_page=block_table[logical_top16]",weights:[]}),
...
function Op({node,active,onHover,onLeave,onSelect}:{node:OpNode;active:boolean;onHover:(n:OpNode)=>void;onLeave:()=>void;onSelect:(n:OpNode)=>void}){
  return <button className={`op-node op-${node.kind} ${active?"active":""}`} onMouseEnter={()=>onHover(node)} onMouseLeave={onLeave} onFocus={()=>onHover(node)} onBlur={onLeave} onClick={()=>onSelect(node)}><b>{node.title}</b><code>{node.outputShape}</code></button>;
}

const Arrow=({label}:{label?:string})=><span className="op-arrow">→{label&&<small>{label}</small>}</span>;
...
function DenseDiagram({g,active,onHover,onLeave,onSelect}:{g:Record<string,OpNode>;active:string;onHover:(n:OpNode)=>void;onLeave:()=>void;onSelect:(n:OpNode)=>void}){
  ...
  return <div className="operator-diagram dense-diagram">
    <div className="flow-row"><N id="input"/><Arrow/><N id="norm"/><Arrow/><N id="qkv"/><Arrow/><N id="split"/></div>
    <div className="branch-box qkv-branches"><div><span>Q branch</span><N id="qnorm"/><Arrow/><N id="ropeq"/></div><div><span>K branch</span><N id="knorm"/><Arrow/><N id="ropek"/><Arrow/><N id="cache"/></div><div><span>V branch</span><N id="cache"/></div><div className="external-input"><b>positions</b><small>[S] → RoPE</small></div><div className="external-input"><b>block_table</b><small>→ KV cache</small></div></div>
    <div className="flow-row attention-row"><N id="qk"/><Arrow/><N id="scale"/><Arrow/><N id="mask"/><Arrow/><N id="softmax"/><Arrow/><N id="pv"/><Arrow/><N id="oproj"/><Arrow/><N id="add1"/></div>
    <div className="side-input"><span>↗</span><b>causal mask</b><small>[S,T]</small></div>
    <div className="flow-row"><N id="postnorm"/><Arrow/><N id="gateup"/><Arrow/><N id="swiglu"/><Arrow/><N id="down"/><Arrow/><N id="add2"/></div>
  </div>;
}
...
function SparseDiagram({g,active,onHover,onLeave,onSelect}:{g:Record<string,OpNode>;active:string;onHover:(n:OpNode)=>void;onLeave:()=>void;onSelect:(n:OpNode)=>void}){
  ...
  return <div className="operator-diagram sparse-diagram">
    <div className="flow-row"><N id="input"/><Arrow/><N id="norm"/><Arrow/><N id="packed"/><Arrow/><N id="split"/></div>
    <div className="dual-path"><section><header>INDEX PATH · 只决定读哪些 block</header><div className="flow-row"><N id="idxnorm"/><Arrow/><N id="idxscore"/><Arrow/><N id="blockmax"/><Arrow/><N id="topk"/></div><div className="path-input">causal mask + local block priority</div></section><section><header>MAIN PATH · 生成精确 attention 输出</header><div className="flow-row"><N id="mainnorm"/><Arrow/><N id="rope"/><Arrow/><N id="cache"/></div><div className="path-input">positions + block_table + Top-16 ids</div></section></div>
    <div className="flow-row attention-row"><N id="qk"/><Arrow/><N id="scale"/><Arrow/><N id="mask"/><Arrow/><N id="softmax"/><Arrow/><N id="pv"/><Arrow/><N id="oproj"/><Arrow/><N id="addattn"/></div>
    <div className="moe-path"><N id="router"/><Arrow/><div className="parallel-ops"><N id="experts"/><N id="shared"/></div><Arrow/><N id="sum"/><Arrow/><N id="addout"/></div>
  </div>;
}
...
export default function Home(){
  ...
  return <main className={`atlas-app ${dark?"dark":""}`}><header className="app-header">
    ...
    <section className="layer-canvas"><header><div><span>SELECTED LAYER DETAIL</span><h1>L{layer} · {layer<3?"Dense GQA + Dense MLP":"MiniMax Sparse Attention + MoE"}</h1></div><code>{layerShard(layer)}</code></header>{layer<3?<DenseDiagram g={graph} active={active.id} onHover={setHovered} onLeave={()=>setHovered(null)} onSelect={setPinned}/>:<SparseDiagram g={graph} active={active.id} onHover={setHovered} onLeave={()=>setHovered(null)} onSelect={setPinned}/>}</section>
